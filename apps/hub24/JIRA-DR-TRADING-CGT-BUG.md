...
### Code pointers
- `apps/hub24/src/pages/adviser/Trading.tsx` — `estimateCgt`
- Unit test covers the discounted estimate: `apps/hub24/src/test/adviser.test.tsx`

### Resolution
1. Restored `* 0.5` in `estimateCgt` so the formula matches the UI copy
2. Updated the trading Vitest to assert `$1,170` instead of documenting `$2,340`
