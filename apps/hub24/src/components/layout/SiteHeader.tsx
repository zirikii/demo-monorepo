...
import { ScamAlertBar } from "./ScamAlertBar";
...
function MegaMenuPanel({ menu, onNavigate }: { menu: MegaMenu; onNavigate: () => void }) {
  return (
    <div className="animate-hub-fade absolute top-full left-0 z-40 w-screen border-t border-line bg-white shadow-hub-menu">
      <div className="container-hub-wide grid gap-8 py-9 lg:grid-cols-[0.85fr_2.15fr]">
        {menu.feature ? (
          <Link
            to={menu.feature.to}
            onClick={onNavigate}
            className="focus-hub flex flex-col justify-between gap-5 rounded-hub-lg bg-hub-navy p-7 text-white transition hover:bg-hub-navy-deep"
          >
            <div className="flex flex-col gap-3">
              ...
              <span className="text-sm leading-relaxed text-white/75">{menu.feature.body}</span>
            </div>
                ...
                {column.links.map((link) => (
                  <li key={link.to + link.label}>
                    <Link
                      to={link.to}
                      onClick={onNavigate}
                      className="focus-hub block rounded-hub px-3 py-2 transition hover:bg-hub-tint"
                    >
                      <span className="flex items-center gap-2 text-[0.95rem] font-bold text-ink-strong">
                        {link.label}
                        {link.badge ? (
                          <span className="rounded-full bg-hub-teal-tint px-2 py-0.5 text-[0.65rem] font-extrabold tracking-wide text-hub-teal-dark uppercase">
                            {link.badge}
                          </span>
                        ) : null}
                      </span>
      ...
      {menu.footerLinks ? (
        <div className="border-t border-line bg-surface-tint py-4">
          <div className="container-hub-wide flex flex-wrap gap-x-8 gap-y-2">
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-line">
        <div className="container-hub-wide flex h-[76px] items-center gap-6">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Link
              ...
              aria-label="Search product documents"
              className="focus-hub hidden rounded-full p-2 text-ink-soft transition hover:text-hub-blue md:inline-flex"
            >
            ...
            <div className="relative hidden md:block">
              ...
              {loginOpen ? (
                <div className="animate-hub-fade absolute right-0 z-40 mt-2 w-72 overflow-hidden rounded-hub-lg border border-line bg-white shadow-hub-menu">
                  {LOGIN_OPTIONS.map((option) => (
                    <Link
                      key={option.to}
                      to={option.to}
                      className="focus-hub block border-b border-line-soft px-5 py-4 transition last:border-b-0 hover:bg-hub-tint"
                    >
      ...
      {mobileOpen ? (
        <div className="animate-hub-fade max-h-[calc(100vh-120px)] overflow-y-auto border-t border-line bg-white lg:hidden">
          <div className="container-hub flex flex-col gap-6 py-6">
            {MEGA_MENUS.map((menu) => (
              <div key={menu.label} className="flex flex-col gap-2">
                <Link to={menu.to} className="focus-hub text-lg font-extrabold text-ink-strong">
                  {menu.label}
                </Link>
                  ...
                  {menu.columns
                    ...
                    .map((link) => (
                      <li key={link.to + link.label}>
                        <Link
                          to={link.to}
                          className="focus-hub block py-1.5 text-[0.95rem] text-ink-soft"
                        >
                          {link.label}
                        </Link>
                      </li>
            ...
            <div className="flex flex-col gap-2 border-t border-line pt-4">
              {SIMPLE_NAV.map((link) => (
                <Link
                  key={link.to}
                  to={link.to}
                  className="focus-hub text-lg font-extrabold text-ink-strong"
                >
                  {link.label}
                </Link>
              ))}
              <Link to="/login" className="focus-hub text-lg font-extrabold text-ink-strong">
                Log in
              </Link>
            </div>
            <ButtonLink to="/contact-us#demo" size="lg" className="w-full">
              Talk to us
            </ButtonLink>
          </div>
        </div>
