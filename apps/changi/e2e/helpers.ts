    ...
    function mount() {
      ...
      style.textContent = `
        #pw-cursor{position:fixed;top:0;left:0;width:22px;height:22px;border-radius:50%;
          background:rgba(238,52,36,0.35);border:2px solid #6c217f;box-shadow:0 0 0 2px rgba(255,255,255,0.7);
          z-index:2147483647;pointer-events:none;transform:translate(-50%,-50%);transition:width .08s,height .08s;}
        .pw-ripple{position:fixed;width:10px;height:10px;border-radius:50%;background:rgba(108,33,127,0.4);
...
export async function moveTo(page: Page, target: Locator) {
  try {
    ...
    /* sticky containers can make this flaky; ignore */
  }
...
/** Move to + click a locator with small pauses for a natural feel. */
 ...
 * the goal is a complete demo video.
 */
...
/** Smooth-scroll the page by a delta and pause. */
