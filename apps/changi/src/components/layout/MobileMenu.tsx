...
export function MobileMenu({ onClose }: MobileMenuProps) {
  ...
  return (
    <nav
      aria-label="Mobile"
      className="max-h-[calc(100vh-4rem)] overflow-y-auto border-t border-sand-deep bg-sand px-4 pb-8 pt-2 lg:hidden"
    >
                  ...
                  {menu.links.map((link) => (
                    <li key={link.to + link.label}>
                      <Link
                        to={link.to}
                        onClick={onClose}
                        className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-ink-soft hover:bg-sand-alt hover:text-magenta"
                      >
                        {link.label}
                      </Link>
                    </li>
      ...
      <div className="mt-6 flex flex-col gap-3">
        <Button to="/rewards" onClick={onClose}>
          Join Changi Rewards
        </Button>
        <Button to="/help" variant="secondary" onClick={onClose}>
          Get help
        </Button>
      </div>
    </nav>
