    ...
    <div>
      ...
      <ul className="mt-6 space-y-4">
        {items.map((item) => (
          ...
          >
            <div>
              <p className="font-semibold text-optus-ink">{item.name}</p>
            ...
            <button
              ...
              className={`relative h-8 w-14 rounded-full transition ${item.connected ? "bg-optus-teal" : "bg-line-strong"}`}
            >
              <span
      ...
      {message ? <p className="mt-4 text-sm text-optus-teal">{message}</p> : null}
    </div>
  );
}
