...
export function SignupForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <p className="rounded-md bg-optus-yellow-light px-3 py-2 text-xs text-optus-ink">
        Demo mode: your account is stored locally only. No real data is collected.
      </p>
      {error ? <p className="text-sm text-red-600">{error}</p> : null}
      <Button type="submit" className="w-full" disabled={pending}>
        {pending ? "Creating account…" : "Create My Optus account"}
      </Button>
      <p className="text-center text-sm text-optus-ink-soft">
        Already with Optus?{" "}
        <Link href="/login" className="font-semibold text-optus-ink hover:underline">
          Sign in
        </Link>
      </p>
    </form>
