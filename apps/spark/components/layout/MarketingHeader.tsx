    ...
    <header className="sticky top-0 z-40 border-b border-line bg-white">
      <div className="container-page flex h-16 items-center justify-between gap-4">
          ...
          <nav className="hidden items-center gap-6 md:flex" aria-label="Primary">
            {MARKETING_NAV.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="text-sm font-medium text-ink-secondary transition-colors hover:text-spark-purple"
              >
                {item.label}
              </Link>
            ))}
        ...
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger
            className="focus-ring inline-flex h-10 w-10 items-center justify-center rounded-md text-spark-ink hover:bg-surface-muted md:hidden"
