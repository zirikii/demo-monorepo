...
The **Sport** page (`/sport`) formats card times from ISO `article.publishedAt` via
`formatRelativeTime` (for example `3h ago`). Latest sort uses `getByPillar("sport")`
so newest stories appear first.
