...
## Sport hub timestamps

The **Sport** page (`/sport`) sorts **Latest** newest-first via `getByPillar("sport")` and
renders relative times from ISO `publishedAt` values (`formatRelativeTime`), matching other
pillars. A Jira ticket in project **DR** (bug label) described the previous NaN timestamp /
inverted-sort defect.
