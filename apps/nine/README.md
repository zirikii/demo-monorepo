...
The **Sport** page (`/sport`) uses ISO `publishedAt` with `formatRelativeTime` and
`getByPillar("sport")` so Latest is newest-first. Card labels are relative times
or en-AU calendar dates — never `NaN hours ago`.
