...
`StoryCard` formats ISO `publishedAt` via `formatRelativeTime`. Seed dates are relative to
`2026-07-16T03:00:00Z`, so later wall-clock dates show en-AU calendar labels rather than
`3h ago`.
