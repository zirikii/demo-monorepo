...
describe("app routes", () => {
  ...
  it("Sport page shows Latest chip and correct timestamps", () => {
    ...
    expect(headings[headings.length - 1]).toHaveTextContent(sport[sport.length - 1].title);

    for (const article of sport) {
      expect(within(grid).getAllByText(formatRelativeTime(article.publishedAt)).length).toBeGreaterThan(0);
    }
  });
