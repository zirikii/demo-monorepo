...
describe("app routes", () => {
  ...
  it("Sport page shows Latest chip, newest-first order, and valid timestamps", async () => {
    ...
    expect(headings[0]?.textContent).toBe(sport[0]?.title);
    expect(headings[headings.length - 1]?.textContent).toBe(sport[sport.length - 1]?.title);

    for (const article of sport) {
      const expected = formatRelativeTime(article.publishedAt);
      expect(expected).not.toMatch(/NaN/i);
      expect(within(grid).getAllByText(expected).length).toBeGreaterThan(0);
    }
  });
