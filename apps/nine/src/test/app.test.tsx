...
describe("app routes", () => {
  ...
  it("Sport page shows Latest chip, newest-first order, and valid timestamps", async () => {
    ...
    expect(titles[titles.length - 1]).toHaveTextContent(sport[sport.length - 1]!.title);

    for (const label of new Set(sport.map((article) => formatRelativeTime(article.publishedAt)))) {
