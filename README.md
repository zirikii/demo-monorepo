...
│   └── spark/    # Vite + React 19 Spark NZ travel packs page (Tailwind v4, TS)
├── packages/
...
| spark | `cn` class merger (`src/lib/cn.ts` re-exports `@demo/ui/cn`) + `<DemoRibbon>` in header |
...
# run one app
pnpm dev:kddi      # or dev:nab / dev:naukri / dev:paytm / dev:seek / dev:spark

# build one app
pnpm build:seek    # or build:kddi / build:nab / build:naukri / build:paytm / build:spark
