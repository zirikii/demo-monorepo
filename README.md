...
│   └── employment-hero/ # Vite + React 19 Employment Hero site + Employment OS preview
└── packages/
...
| employment-hero | `cn` class merger (`src/lib/cn.ts` re-exports `@demo/ui/cn`) + `<DemoRibbon>` in header and product preview |
...
# run one app
pnpm dev:kddi      # or dev:nab / … / optus / commbank / employment-hero
