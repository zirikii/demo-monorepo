...
```
.cursor/skills/mock-company-repo/prompts/
```
...
Write **one file only** to `.cursor/skills/mock-company-repo/prompts/<slug>.md` using the template below, filled with researched specifics. Replace every `{{PLACEHOLDER}}`. The prompt must be standalone, robust, and unambiguous since it runs unattended.
...
**Prompt-only mode:** tell the user the saved path (e.g. `.cursor/skills/mock-company-repo/prompts/kddi-noc-dashboard.md`) and how to run it downstream:
...
- All prompt files live in `.cursor/skills/mock-company-repo/prompts/` — never scatter them across app repos or the home directory.
