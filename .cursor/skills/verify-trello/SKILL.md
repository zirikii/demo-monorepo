...
Project-local skill for the mock **Trello** workspace in `apps/atlassian`. Marketing routes and other product apps are out of scope. There is no backend: browser + `localStorage` only.
...
export VERIFY_TRELLO_URL=http://localhost:53183
```

Ready when that log shows `Local: http://localhost:53183/`. Record that PID. Doctor and drive must use `VERIFY_TRELLO_URL`.
...
```bash
bash .cursor/skills/verify-trello/scripts/doctor.sh
```

Override the origin with `VERIFY_TRELLO_URL` when you launched on an isolated port.

Must pass:

1. **Port.** Something is listening on the origin's TCP port (5183 by default, or the port in `VERIFY_TRELLO_URL`). The listener is this run's Vite. If the Vite log printed a different port than you requested, fail.
2. **App identity.** `GET $ORIGIN/` body contains `Atlassian (Demo)` and does not look like another customer demo.
3. **Portal.** `GET $ORIGIN/login?portal=trello` returns 200. The heading `Log in to Trello` is client-rendered — prove it in the drive, not curl.
4. **Workspace, not marketing.** After auth, the URL path is `/trello` (local) or `/atlassian/trello` (deployed). Visible chrome: nav `aria-label="Trello"`, heading `Northline launch`.
...
| Handle | Where |
|---|---|
| heading `Log in to Trello` | `/login?portal=trello` |
| textbox `Email`, textbox `Password` | login form, pre-filled |
| button `Continue` | submit login |
| button `Switch apps` | product switcher |
| heading `Northline launch` | `/trello` |
| headings `Inbox`, `Doing`, `Review`, `Done` | lists |
| article `Draft press kit` | card |
| combobox `Move Draft press kit` | list move (accessible) |
| HTML5 drop on a list | drag move |

Seeded login: `demo@atlassian.com` / `teamwork2026` → `/trello`.
...
Write under `.cursor/skills/verify-trello/artifacts/<run-id>/`. Cleanup must not delete this directory. Git ignores the files; they still belong on disk after teardown.
...
- After a mutation, read `atlassian-demo-trello`, then full reload, then read it again.
...
- Leave `.cursor/skills/verify-trello/artifacts/` in place.
- Do not `localStorage.clear()` unless a feature file says to reset seed (`removeItem('atlassian-demo-trello')` only).

## Helpers

- `bash .cursor/skills/verify-trello/scripts/doctor.sh` — port + HTML identity + Trello login portal.
- `node .cursor/skills/verify-trello/scripts/drive.mjs` — isolated Chromium default proof. Honors `VERIFY_TRELLO_URL` and `VERIFY_TRELLO_ARTIFACTS`. If Chromium is missing: `pnpm --filter nab exec playwright install chromium`.
