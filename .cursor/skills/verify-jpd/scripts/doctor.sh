...
URL="${VERIFY_JPD_URL:-http://localhost:5183}"
URL="${URL%/}/"
LOGIN="${URL}login?portal=jpd"

echo "doctor: GET $URL"
code="$(curl -sS -o /tmp/verify-jpd-doctor.html -w '%{http_code}' "$URL")"
if [[ "$code" != "200" ]]; then
  echo "doctor: fail HTTP $code" >&2
  exit 1
fi
if ! grep -q 'Atlassian (Demo)' /tmp/verify-jpd-doctor.html; then
  echo "doctor: fail — response is not the Atlassian demo (missing 'Atlassian (Demo)')" >&2
  exit 1
fi

echo "doctor: GET $LOGIN"
login_code="$(curl -sS -o /tmp/verify-jpd-login.html -w '%{http_code}' "$LOGIN")"
...
echo "doctor: ok — Jira Product Discovery portal is answering at $URL"
