---
name: verify-jpd
description: Drive the Atlassian demo's mock Jira Product Discovery workspace (apps/atlassian, port 5183) — login, ideas, votes, status. Use when proving JPD, not Jira delivery.
---

# Verify Jira Product Discovery (Atlassian demo)

Project-local skill for the mock **Jira Product Discovery** workspace in `apps/atlassian`. Marketing routes and other product apps are out of scope. There is no backend: browser + `localStorage` only.

Do not create or follow a `verify-atlassian` skill. This one is Jira Product Discovery-workspace scoped.
...
export VERIFY_JPD_URL=http://localhost:53183
```

Ready when that log shows `Local: http://localhost:53183/`. Record that PID. Doctor and drive must use `VERIFY_JPD_URL`.
...
```bash
bash .cursor/skills/verify-jpd/scripts/doctor.sh
```

Override the origin with `VERIFY_JPD_URL` when you launched on an isolated port.

Must pass:

1. **Port.** Something is listening on the origin's TCP port (5183 by default, or the port in `VERIFY_JPD_URL`). The listener is this run's Vite. If the Vite log printed a different port than you requested, fail.
2. **App identity.** `GET $ORIGIN/` body contains `Atlassian (Demo)` and does not look like another customer demo.
3. **Portal.** `GET $ORIGIN/login?portal=jpd` returns 200. The heading `Log in to Jira Product Discovery` is client-rendered — prove it in the drive, not curl.
4. **Workspace, not marketing.** After auth, the URL path is `/jpd` (local) or `/atlassian/jpd` (deployed). Visible chrome: nav `aria-label="Jira Product Discovery"`, heading `Ideas`.
...
| Handle | Where |
|---|---|
| heading `Log in to Jira Product Discovery` | `/login?portal=jpd` |
| textbox `Email`, textbox `Password` | login form, pre-filled |
| button `Continue` | submit login |
| button `Switch apps` | product switcher |
| heading `Ideas` | `/jpd` |
| link `DISC-9` | idea detail |
| button `Vote for DISC-9` | vote |
| combobox/select `Status` (`#idea-status`) | idea detail |

Seeded login: `demo@atlassian.com` / `teamwork2026` → `/jpd`.
...
Write under `.cursor/skills/verify-jpd/artifacts/<run-id>/`. Cleanup must not delete this directory. Git ignores the files; they still belong on disk after teardown.
...
- After a mutation, read `atlassian-demo-ideas`, then full reload, then read it again.
...
- Leave `.cursor/skills/verify-jpd/artifacts/` in place.
- Do not `localStorage.clear()` unless a feature file says to reset seed (`removeItem('atlassian-demo-ideas')` only).

## Helpers

- `bash .cursor/skills/verify-jpd/scripts/doctor.sh` — port + HTML identity + Jira Product Discovery login portal.
- `node .cursor/skills/verify-jpd/scripts/drive.mjs` — isolated Chromium default proof. Honors `VERIFY_JPD_URL` and `VERIFY_JPD_ARTIFACTS`. If Chromium is missing: `pnpm --filter nab exec playwright install chromium`.
