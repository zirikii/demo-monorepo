---
name: verify-jsm
description: Drive the Atlassian demo's mock Jira Service Management workspace (apps/atlassian, port 5183) — login, queues, request status. Use when proving JSM, not Jira software.
---

# Verify Jira Service Management (Atlassian demo)

Project-local skill for the mock **Jira Service Management** workspace in `apps/atlassian`. Marketing routes and other product apps are out of scope. There is no backend: browser + `localStorage` only.

Do not create or follow a `verify-atlassian` skill. This one is Jira Service Management-workspace scoped.
...
export VERIFY_JSM_URL=http://localhost:53183
```

Ready when that log shows `Local: http://localhost:53183/`. Record that PID. Doctor and drive must use `VERIFY_JSM_URL`.
...
```bash
bash .cursor/skills/verify-jsm/scripts/doctor.sh
```

Override the origin with `VERIFY_JSM_URL` when you launched on an isolated port.

Must pass:

1. **Port.** Something is listening on the origin's TCP port (5183 by default, or the port in `VERIFY_JSM_URL`). The listener is this run's Vite. If the Vite log printed a different port than you requested, fail.
2. **App identity.** `GET $ORIGIN/` body contains `Atlassian (Demo)` and does not look like another customer demo.
3. **Portal.** `GET $ORIGIN/login?portal=jsm` returns 200. The heading `Log in to Jira Service Management` is client-rendered — prove it in the drive, not curl.
4. **Workspace, not marketing.** After auth, the URL path is `/jsm` (local) or `/atlassian/jsm` (deployed). Visible chrome: nav `aria-label="Jira Service Management"`, heading `Queues`.
...
| Handle | Where |
|---|---|
| heading `Log in to Jira Service Management` | `/login?portal=jsm` |
| textbox `Email`, textbox `Password` | login form, pre-filled |
| button `Continue` | submit login |
| button `Switch apps` | product switcher |
| heading `Queues` | `/jsm` |
| tab `Incident` / `Service request` / `Change` | queue filter |
| link whose name contains `ITSM-14` | request detail |
| combobox/select `Status` (`#request-status`) | request detail |

Seeded login: `demo@atlassian.com` / `teamwork2026` → `/jsm`.
...
Write under `.cursor/skills/verify-jsm/artifacts/<run-id>/`. Cleanup must not delete this directory. Git ignores the files; they still belong on disk after teardown.
...
- After a mutation, read `atlassian-demo-requests`, then full reload, then read it again.
...
- Leave `.cursor/skills/verify-jsm/artifacts/` in place.
- Do not `localStorage.clear()` unless a feature file says to reset seed (`removeItem('atlassian-demo-requests')` only).

## Helpers

- `bash .cursor/skills/verify-jsm/scripts/doctor.sh` — port + HTML identity + Jira Service Management login portal.
- `node .cursor/skills/verify-jsm/scripts/drive.mjs` — isolated Chromium default proof. Honors `VERIFY_JSM_URL` and `VERIFY_JSM_ARTIFACTS`. If Chromium is missing: `pnpm --filter nab exec playwright install chromium`.
