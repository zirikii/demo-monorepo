---
name: verify-admin
description: Drive the Atlassian demo's mock Admin hub (apps/atlassian, port 5183) — login, directory, organisation policy toggles. Use when proving /admin, not Jira project settings.
---

# Verify Admin (Atlassian demo)

Project-local skill for the mock **Admin** workspace in `apps/atlassian`. Marketing routes and other product apps are out of scope. There is no backend: browser + `localStorage` only.
...
export VERIFY_ADMIN_URL=http://localhost:53183
```

Ready when that log shows `Local: http://localhost:53183/`. Record that PID. Doctor and drive must use `VERIFY_ADMIN_URL`.
...
```bash
bash .cursor/skills/verify-admin/scripts/doctor.sh
```

Override the origin with `VERIFY_ADMIN_URL` when you launched on an isolated port.

Must pass:

1. **Port.** Something is listening on the origin's TCP port (5183 by default, or the port in `VERIFY_ADMIN_URL`). The listener is this run's Vite. If the Vite log printed a different port than you requested, fail.
2. **App identity.** `GET $ORIGIN/` body contains `Atlassian (Demo)` and does not look like another customer demo.
3. **Portal.** `GET $ORIGIN/login?portal=admin` returns 200. The heading `Log in to Admin` is client-rendered — prove it in the drive, not curl.
4. **Workspace, not marketing.** After auth, the URL path is `/admin` (local) or `/atlassian/admin` (deployed). Visible chrome: nav `aria-label="Admin"`, heading `Admin`.
5. **Auth.** Seeded credentials `admin@atlassian.com` / `admin2026`. This app rejects unknown emails and wrong passwords.
...
| Handle | Where |
|---|---|
| heading `Log in to Admin` | `/login?portal=admin` |
| textbox `Email`, textbox `Password` | login form, pre-filled |
| button `Continue` | submit login |
| button `Switch apps` | product switcher |
| heading `Admin` | `/admin` |
| text `Maya Chen` | directory |
| switch `Two-step verification` | policies |
| switch `Public signup` | policies |

Seeded login: `admin@atlassian.com` / `admin2026` → `/admin`.
...
Write under `.cursor/skills/verify-admin/artifacts/<run-id>/`. Cleanup must not delete this directory. Git ignores the files; they still belong on disk after teardown.
...
- After a mutation, read `atlassian-demo-admin`, then full reload, then read it again.
...
- Leave `.cursor/skills/verify-admin/artifacts/` in place.
- Do not `localStorage.clear()` unless a feature file says to reset seed (`removeItem('atlassian-demo-admin')` only).

## Helpers

- `bash .cursor/skills/verify-admin/scripts/doctor.sh` — port + HTML identity + Admin login portal.
- `node .cursor/skills/verify-admin/scripts/drive.mjs` — isolated Chromium default proof. Honors `VERIFY_ADMIN_URL` and `VERIFY_ADMIN_ARTIFACTS`. If Chromium is missing: `pnpm --filter nab exec playwright install chromium`.
