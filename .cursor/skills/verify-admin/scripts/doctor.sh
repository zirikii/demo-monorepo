...
URL="${VERIFY_ADMIN_URL:-http://localhost:5183}"
URL="${URL%/}/"
LOGIN="${URL}login?portal=admin"

echo "doctor: GET $URL"
code="$(curl -sS -o /tmp/verify-admin-doctor.html -w '%{http_code}' "$URL")"
if [[ "$code" != "200" ]]; then
  echo "doctor: fail HTTP $code" >&2
  exit 1
fi
if ! grep -q 'Atlassian (Demo)' /tmp/verify-admin-doctor.html; then
  echo "doctor: fail — response is not the Atlassian demo (missing 'Atlassian (Demo)')" >&2
  exit 1
fi

echo "doctor: GET $LOGIN"
login_code="$(curl -sS -o /tmp/verify-admin-login.html -w '%{http_code}' "$LOGIN")"
...
echo "doctor: ok — Admin portal is answering at $URL"
