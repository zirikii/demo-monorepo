...
- Board drag (`move-drag`) is the demo target even while the board cannot drop cards. Report it blocked until `Board.tsx` grows drop targets. Do not report issue-detail Status as that proof.
...
- [Move issue](./move-issue.md) — target demo proof: drag a card; persist via `updateIssueStatus` / `atlassian-demo-issues`. Board drag is **intentionally blocked** for demos; Status select on issue detail is the shipped equivalent and must be labeled as such.
